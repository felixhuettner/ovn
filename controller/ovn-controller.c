...
loop_done:
    ...
    /* We first stop and cleanup pinctrl. Since that is a separate thread that
     * could access data asynchronously. */
    pinctrl_destroy();

    /* Now we cleanup the engine, since it uses data from a lot of others. */
    engine_cleanup();

    /* And now we can cleanup the rest. */
    free(ovn_version);
    lflow_destroy();
    ofctrl_destroy();
